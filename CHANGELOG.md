...
- On Windows, tabs are also frozen when free memory drops below 15%. On Linux
  this now works too, in principle - the code path exists and applies
  cleanly, but it has not been exercised on real hardware under real memory
  pressure yet, so treat it as provisional until a release confirms it.
