...
If you find a security issue in code specific to Aerium — the patches in `patches/aerium-fatih/`, the branding sweep in `scripts/apply_branding.py`, or the AppImage packaging in `scripts/package.sh` — please don't open a public issue. Instead, use [GitHub Security Advisories](https://github.com/aerium-browser/aerium-browser-linux/security/advisories/new) for this repo.
...
Vulnerabilities in Chromium itself, in ungoogled-chromium's own patches, or in the AppImage packaging mechanism inherited from `ungoogled-chromium-portablelinux` aren't Aerium-specific — report those upstream instead:
...
- ungoogled-chromium-portablelinux (AppImage packaging): [their issue tracker](https://github.com/ungoogled-software/ungoogled-chromium-portablelinux/issues)
