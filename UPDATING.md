...
3. Resolve conflicts. Files most likely to conflict, and which side to prefer:
   ...
   - `scripts/package.sh` — keep the `aerium`/`fatih-gh`/`aerium-browser-linux` renames; take upstream's changes to the file list, AppDir layout, or packaging logic itself.
